...
<div style="background-color: #eeeeee; padding: 40px 20px 100px 20px; border-radius: 10px; display: flex; flex-direction: column; align-items: center; font-family: 'Malgun Gothic', sans-serif;">
    ...
    <div style="position: relative; width: 758px; height: 450px; margin-top: 50px;">
"""

# 1. 사각형 배치 (A20x, A40x) - 상하 간격 제거(160-2=158)
for r_idx, row in enumerate(rect_rows):
    y_pos = r_idx * (RECT_H - OVERLAP) 
    for c_idx, name in enumerate(row):
        x_pos = c_idx * (RECT_W - OVERLAP)
        final_html += f'''
...
# 2. 동그라미 배치 (경계 라인 밀착 정렬)
# y_offsets: 상단(-44), 중간(160-2-44=114), 하단(320-4-44=272)
y_offsets = [-44, 114, 272]

for r_idx, row in enumerate(circle_rows):
    for c_idx, name in enumerate(row):
        x_pos = (c_idx + 1) * (RECT_W - OVERLAP) - 44
        final_html += f'''
        <div style="position: absolute; left: {x_pos}px; top: {y_offsets[r_idx]}px; width: {CIRC_D}px; height: {CIRC_D}px; border: 2px solid #333; border-radius: 50%; background: white; display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 2; box-shadow: 2px 2px 5px rgba(0,0,0,0.1);">
